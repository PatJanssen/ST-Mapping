...
{ 2017-01-31   Patched by PWMJ to work with Lazarus under 64-bit Windows OS
...
uses
 Clipbrd, LCLIntf, LCLType, Windows {$IFDEF UseDelphi}, Classes, Graphics, SysUtils{$ENDIF}, PasZLib, pnglang;
...
type
  ...
  public
    ...
    procedure LoadFromResourceID(Instance: HInst; ResID: PtrUInt);
...
{Used to invert bytes using assembly, but PWMJ changed for Lazarus 64bit}
function ByteSwap(const a: integer): integer;
//{$ASMMODE intel}
//asm
//  bswap eax
begin
  ByteSwap := SwapEndian(a);
end;
...
{    InflateInit_(StreamRec, zlib_version, SIZEOF(TZStreamRec));}
    ...
    repeat
      ...
      begin
        ...
        {Copies the new data}
        CopyMemory(Pointer(PtrUInt(Output) + OutputSize - total_out),
          @Buffer, total_out);
...
begin
  ...
  {Reads data}
  fYear := ((pByte(PtrUInt(Data) )^) * 256)+ (pByte(PtrUInt(Data) + 1)^);
  fMonth := pByte(PtrUInt(Data) + 2)^;
  fDay := pByte(PtrUInt(Data) + 3)^;
  fHour := pByte(PtrUInt(Data) + 4)^;
  fMinute := pByte(PtrUInt(Data) + 5)^;
  fSecond := pByte(PtrUInt(Data) + 6)^;
end;
...
begin
  ...
  pByte(PtrUInt(Data) + 2)^ := Month;
  pByte(PtrUInt(Data) + 3)^ := Day;
  pByte(PtrUInt(Data) + 4)^ := Hour;
  pByte(PtrUInt(Data) + 5)^ := Minute;
  pByte(PtrUInt(Data) + 6)^ := Second;
...
begin
  ...
  if PtrUInt(fKeyword) = 0 then
    CompressionMethod := pByte(Data)^
  else
    CompressionMethod := pByte(PtrUInt(fKeyword) + Length(fKeyword))^;
  fText := '';
  ...
  begin
    Output := nil;
    if DecompressZLIB(PChar(PtrUInt(Data) + Length(fKeyword) + 2),
...
begin
  ...
  begin
    ...
    pByte(Pointer(PtrUInt(Data) + Length(Keyword) + 1))^ := 0;
    if OutputSize > 0 then
      CopyMemory(Pointer(PtrUInt(Data) + Length(Keyword) + 2), Output, OutputSize);
...
begin
  ...
  CopyMemory(@fText[1], Pointer(PtrUInt(Data) + Length(fKeyword) + 1),
    Length(fText));
end;
...
begin
  ...
  if Keyword <> '' then
    CopyMemory(Data, @fKeyword[1], Length(Keyword));
  if Text <> '' then
    CopyMemory(Pointer(PtrUInt(Data) + Length(Keyword) + 1), @fText[1],
      Length(Text));
  {Let ancestor calculate crc and save}
  Result := inherited SaveToStream(Stream);
end;
...
begin
  ...
  Dest := pChar(PtrUInt(Dest) + Col * 3);
  repeat
    {Copy this row}
    Byte(Dest^) := fOwner.GammaTable[pByte(PtrUInt(Src) + 2)^]; inc(Dest);
    Byte(Dest^) := fOwner.GammaTable[pByte(PtrUInt(Src) + 1)^]; inc(Dest);
    Byte(Dest^) := fOwner.GammaTable[pByte(PtrUInt(Src)    )^]; inc(Dest);
...
begin
  ...
  repeat
    {Copy this row}
    Byte(Dest^) := Owner.GammaTable[pByte(PtrUInt(Src) + 4)^]; inc(Dest);
    Byte(Dest^) := Owner.GammaTable[pByte(PtrUInt(Src) + 2)^]; inc(Dest);
    Byte(Dest^) := Owner.GammaTable[pByte(PtrUInt(Src)    )^]; inc(Dest);
    {$IFDEF Store16bits}
    {Copy extra pixel values}
    Byte(Extra^) := fOwner.GammaTable[pByte(PtrUInt(Src) + 5)^]; inc(Extra);
    Byte(Extra^) := fOwner.GammaTable[pByte(PtrUInt(Src) + 3)^]; inc(Extra);
    Byte(Extra^) := fOwner.GammaTable[pByte(PtrUInt(Src) + 1)^]; inc(Extra);
    {$ENDIF}
...
begin
  ...
  repeat
    ...
    repeat
      ...
      Dest2 := pChar(PtrUInt(Dest) + (Header.BitDepth * Col) div 8);
      {Copy data}
...
begin
  ...
  repeat
    ...
    repeat
      {Adjust pointer to pixel byte bounds}
      Dest2 := pChar(PtrUInt(Dest) + Col div 2);
      {Copy data}
...
begin
  {Get first column and enter in loop}
  Col := ColumnStart[Pass];
  Dest := pChar(PtrUInt(Dest) + Col);
  repeat
    {Copy this row}
    Dest^ := Src^; inc(Dest);
    {$IFDEF Store16bits}
    Extra^ := pChar(PtrUInt(Src) + 1)^; inc(Extra);
    {$ENDIF}
...
begin
  ...
  Col := ColumnStart[Pass];
  Dest := pChar(PtrUInt(Dest) + Col * 3);
  Trans := pChar(PtrUInt(Trans) + Col);
  repeat
    {Copy this row and alpha value}
    Trans^ := pChar(PtrUInt(Src) + 3)^;
    Byte(Dest^)  := fOwner.GammaTable[pByte(PtrUInt(Src) + 2)^]; inc(Dest);
    Byte(Dest^)  := fOwner.GammaTable[pByte(PtrUInt(Src) + 1)^]; inc(Dest);
    Byte(Dest^)  := fOwner.GammaTable[pByte(PtrUInt(Src)    )^]; inc(Dest);
...
begin
  ...
  repeat
    {Copy this row and alpha value}
    Trans^ := pChar(PtrUInt(Src) + 6)^;
    Byte(Dest^)  := fOwner.GammaTable[pByte(PtrUInt(Src) + 4)^]; inc(Dest);
    Byte(Dest^)  := fOwner.GammaTable[pByte(PtrUInt(Src) + 2)^]; inc(Dest);
    Byte(Dest^)  := fOwner.GammaTable[pByte(PtrUInt(Src)    )^]; inc(Dest);
    {$IFDEF Store16bits}
    {Copy extra pixel values}
...
begin
  ...
  begin
    ...
    Data := Pointer(PtrUInt(Header.ImageData) + Header.BytesPerRow *
      (ImageHeight - 1 - CurrentRow));
    Trans := Pointer(PtrUInt(Header.ImageAlpha) + ImageWidth * CurrentRow);
    {$IFDEF Store16bits}
    Extra := Pointer(PtrUInt(Header.ExtraImageData) + Header.BytesPerRow *
      (ImageHeight - 1 - CurrentRow));
    {$ENDIF}
...
begin
  ...
  PtrUInt(Data) := PtrUInt(Header.ImageData) +
    Header.BytesPerRow * (ImageHeight - 1);
  Trans := Header.ImageAlpha;
  {$IFDEF Store16bits}
  PtrUInt(Extra) := PtrUInt(Header.ExtraImageData) +
    Header.BytesPerRow * (ImageHeight - 1);
  {$ENDIF}
  {Reads each line}
...
begin
  ...
  begin
    {Copy pixel values}
    Byte(Dest^) := fOwner.InverseGamma[pByte(PtrUInt(Src) + 2)^]; inc(Dest);
    Byte(Dest^) := fOwner.InverseGamma[pByte(PtrUInt(Src) + 1)^]; inc(Dest);
    Byte(Dest^) := fOwner.InverseGamma[pByte(PtrUInt(Src)    )^]; inc(Dest);
    {Move to next pixel}
    inc(Src, 3);
  end {for I}
end;
...
begin
  ...
  begin
    ...
    //for sample
    {Copy pixel values}
    pWORD(Dest)^ := fOwner.InverseGamma[pByte(PtrUInt(Src) + 2)^]; inc(Dest, 2);
    pWORD(Dest)^ := fOwner.InverseGamma[pByte(PtrUInt(Src) + 1)^]; inc(Dest, 2);
    pWORD(Dest)^ := fOwner.InverseGamma[pByte(PtrUInt(Src)    )^]; inc(Dest, 2);
    {Move to next pixel}
    inc(Src, 3);
  end {for I}
...
begin
  ...
  begin
    //Now we copy from 1 byte for each sample stored to a 2 bytes (or 1 word)
    //for sample
    pWORD(Dest)^ := pByte(PtrUInt(Src))^; inc(Dest, 2);
    {Move to next pixel}
    inc(Src);
  end {for I}
end;
...
begin
  ...
  FOR i := 1 TO ImageWidth do
  begin
    Byte(Dest^) := Owner.InverseGamma[PByte(PtrUInt(Src) + 2)^]; inc(Dest);
    Byte(Dest^) := Owner.InverseGamma[PByte(PtrUInt(Src) + 1)^]; inc(Dest);
    Byte(Dest^) := Owner.InverseGamma[PByte(PtrUInt(Src)    )^]; inc(Dest);
    Dest^ := Trans^; inc(Dest);
    inc(Src, 3); inc(Trans);
  end {for i};
end;
...
begin
  ...
  FOR i := 1 TO ImageWidth do
  begin
    pWord(Dest)^ := Owner.InverseGamma[PByte(PtrUInt(Src) + 2)^]; inc(Dest, 2);
    pWord(Dest)^ := Owner.InverseGamma[PByte(PtrUInt(Src) + 1)^]; inc(Dest, 2);
    pWord(Dest)^ := Owner.InverseGamma[PByte(PtrUInt(Src)    )^]; inc(Dest, 2);
    pWord(Dest)^ := PByte(PtrUInt(Trans)  )^; inc(Dest, 2);
    inc(Src, 3); inc(Trans);
  end {for i};
end;
...
begin
  ...
  with Header.BitmapInfo.bmiHeader do
    repeat
      ...
      repeat
        {Adjust pointer to pixel byte bounds}
        Src2 := pChar(PtrUInt(Src) + (biBitCount * Col) div 8);
        {Copy data}
...
begin
  ...
  repeat
    {Copy this row}
    pWord(Dest)^ := pByte(PtrUInt(Src) + 2)^; inc(Dest, 2);
    pWord(Dest)^ := pByte(PtrUInt(Src) + 1)^; inc(Dest, 2);
    pWord(Dest)^ := pByte(PtrUInt(Src)    )^; inc(Dest, 2);
    pWord(Dest)^ := pByte(Trans)^; inc(Dest, 2);
...
begin
  ...
  PtrUInt(ImageSource) := PtrUInt(Header.ImageData) +
    Header.BytesPerRow * PtrUInt(Header.Height - 1);
  ...
  case Header.BitmapInfo.bmiHeader.biBitCount of
    ...
    24:
      ...
      begin
        ...
        {Move pointers}
        inc(PtrUInt(ImageData), BytesPerRowDest);
        if Stretch then j2 := trunc(j / FactorY) else j2 := j;
        PtrUInt(ImageSource) := PtrUInt(ImageSourceOrg) - BytesPerRowSrc * j2;
        PtrUInt(AlphaSource) := PtrUInt(Header.ImageAlpha) +
          BytesPerRowAlpha * j2;
      end;
    ...
    1,4,8: if Header.ColorType = COLOR_GRAYSCALEALPHA then
      ...
      begin
        ...
        {Move pointers}
        PtrUInt(ImageData) := PtrUInt(ImageData) + BytesPerRowDest;
        if Stretch then j2 := trunc(j / FactorY) else j2 := j;
        PtrUInt(ImageSource) := PtrUInt(ImageSourceOrg) - BytesPerRowSrc * j2;
        PtrUInt(AlphaSource) := PtrUInt(Header.ImageAlpha) +
          BytesPerRowAlpha * j2;
...
{Saves to clipboard format}
procedure TPNGObject.SaveToClipboard;
var Bitmap : TBitmap;
begin
  Bitmap := TBitmap.Create;
  try
    Self.AssignTo(Bitmap);
...
function TPngObject.LoadFromClipboard : boolean;
var Bitmap : TBitmap;
begin
  Result := false;
  Bitmap := TBitmap.Create;
  if Clipboard.HasFormat(PredefinedClipboardFormat(pcfDelphiBitmap)) or
    ...
    try
      ...
      Self.AssignHandle(Bitmap.Handle, False, 0);
      Result := true;
    finally
      Bitmap.Free;
    end; {try}
...
procedure TPngObject.LoadFromResourceID(Instance: HInst; ResID: PtrUInt);
begin
...
begin
  with Header do
    if (ColorType = COLOR_RGBALPHA) or (ColorType = COLOR_GRAYSCALEALPHA) then
      PtrUInt(Result) := PtrUInt(ImageAlpha) + (LineIndex * PtrUInt(Width))
...
begin
  with Header do
    PtrUInt(Result) := (PtrUInt(ExtraImageData) + ((PtrUInt(Height) - 1) *
...
begin
  with Header do
    PtrUInt(Result) := (PtrUInt(ImageData) + ((PtrUInt(Height) - 1) *
      BytesPerRow)) - (LineIndex * BytesPerRow);
end;
...
begin
  ...
  begin
    ...
    for Line := 0 to Min(CY - 1, Height - 1) do
      CopyMemory(Pointer(PtrUInt(NewImageData) + (PtrUInt(CY) - 1) *
    ...
    begin
      ...
      for Line := 0 to Min(CY - 1, Height - 1) do
        CopyMemory(Pointer(PtrUInt(NewImageAlpha) + (Line * CX)),
    ...
    begin
      ...
      for Line := 0 to Min(CY - 1, Height - 1) do
        CopyMemory(Pointer(PtrUInt(NewImageExtra) + (Line * CX)),
...
begin
  ...
  {Reads data}
  fPPUnitX := ByteSwap(pCardinal(PtrUInt(Data))^);
  fPPUnitY := ByteSwap(pCardinal(PtrUInt(Data) + 4)^);
  fUnit := pUnitType(PtrUInt(Data) + 8)^;
end;
...
begin
  ...
  pCardinal(PtrUInt(Data) + 4)^ := ByteSwap(fPPUnitY);
  pUnitType(PtrUInt(Data) + 8)^ := fUnit;
